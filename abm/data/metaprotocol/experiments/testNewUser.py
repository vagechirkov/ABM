...
Date:       31.05.2022
Parameters: expFig1p3: After the 17 preliminary experiments we decided on the following experimental setting:
...
Experiment: In this experiment we use the above decribed golden standard parameters for simulations with:
                - 10 agents
                - Realistic vision: WITH occlusions and LIMITED FOV
                - No further physical constraints (idealized system, i.e. Ghost Mode off) other than general particle-like
                collision avoidance                
...
criteria_exp = [
    Constant("N", 10),
    Constant("VISUAL_EXCLUSION", 1),  # with visual occlusion
    Constant("AGENT_FOV", 0.5),  # limited
    Constant("GHOST_WHILE_EXPLOIT", 1),
    Tunable("DEC_EPSW", values_override=[0, 1, 5]),
    Constant("DEC_EPSU", 1),
